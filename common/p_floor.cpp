		...
		if (m_Type != waitStair || m_ResetCount == 0)
		{
			if (m_Direction == 1)
			{
				...
				case genFloorChg0:
					...
					//fall thru
				case genFloorChg:
					m_Sector->floor_texhandle = m_Texture;
					break;
				default:
					break;
				}
	...
	: DMovingFloor (sec), m_Status(init)
{
	...
	case DFloor::floorRaiseAndChange:
		...
		if (line)
		{
			sec->floor_texhandle = line->frontsector->floor_texhandle;
	...
	if (change & 3)
	{
		...
		if (change & 4)
		{
			...
			m_Texture = line->frontsector->floor_texhandle;
...
BOOL EV_DoChange (line_t *line, EChange changetype, int tag)
{
	...
	while ((secnum = P_FindSectorFromTag (tag, secnum)) >= 0)
	{
		...
		case numChangeOnly:
			...
			{
				sec->floor_texhandle = secm->floor_texhandle;
		...
		do
		{
			...
			else
			{
				for (i = 0; i < sec->linecount; i++)
				{
					...
					if (!igntxt && tsec->floor_texhandle != texture)
						continue;
...
int EV_DoDonut (int tag, fixed_t pillarspeed, fixed_t slimespeed)
{
	...
	while ((secnum = P_FindSectorFromTag(tag,secnum)) >= 0)
	{
		...
		for (i = 0; i < s2->linecount; i++)
		{
			...
			floor->m_Texture = s3->floor_texhandle;
